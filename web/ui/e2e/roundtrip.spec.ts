...
type WasmWindow = {
  Module?: { _wasm_set_canvas_size?: unknown; _maple_bridge_send?: unknown };
};
...
test("overlay mounts and shows a scene", async ({ page }) => {
  ...
  await expect(probe).toContainText(/scene: (login|ingame|loading)/, {
    timeout: 15000,
  });
});
...
test("ping round-trips through the engine", async ({ page }) => {
  ...
  await page.waitForFunction(
    () =>
      typeof (window as unknown as WasmWindow).Module?._maple_bridge_send ===
      "function",
    null,
    { timeout: 25000 },
  );
  ...
  await page.evaluate(() =>
    (
      document.querySelector(".phase0-probe button") as HTMLButtonElement
    )?.click(),
  );
  await expect(page.locator(".phase0-probe")).toContainText(/last pong: \d+/, {
    timeout: 5000,
  });
});
...
test("live stats reach the overlay in-game", async ({ page }) => {
  ...
  const L = 40,
    S = 1.5,
    g = (x: number, y: number): [number, number] => [L + x * S, y * S];
